class Grid {
  ...
  openAdjoiningSpaces(space) {
    if (space.isEmpty) {
      let loop = true
      var toOpen = [
        ...this.getBorderingSpaces(space).filter((space) => !space.hasMine),
      ]
      while (loop) {
  ...
  drawGrid() {
    ...
    this.spaces.forEach((row) => {
      row.forEach(
        (space) => (grid += `<div class="cell" id=${space.id}></div>`)
      )
      grid += "<br>"
    })
  ...
  addMines(numberOfMines) {
    ...
    for (let i = 0; i < numberOfMines; i++) {
      const randomSpace =
        copyOfSpaces[Math.floor(Math.random() * copyOfSpaces.length)]
